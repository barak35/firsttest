...
class korbit:
    ...
    ETC = int(FindETC)
    reqXRP = Request('https://api.korbit.co.kr/v1/ticker/detailed?currency_pair=xrp_krw' , headers={'User-Agent': 'Mozilla/5.0'})
    readXRP = urlopen(reqXRP).read()
    jsonXRP = json.loads(readXRP)
    lastXRP = int(jsonXRP['last'])
    lowXRP = int(jsonXRP['low'])
    highXRP = int(jsonXRP['high'])